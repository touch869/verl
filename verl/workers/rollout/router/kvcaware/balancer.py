...
class KVCAwareBalancer:
    ...
    def __init__(self, servers: dict[str, Any], config: Optional[dict] = None) -> None:
        ...
        self._route_time_since_flush = 0
    ...
    def acquire_server(self, request_id: str, prompt_ids: list[int] | None = None) -> tuple[str, Any]:
        ...
        self._route_time_since_flush += 1
        if not ranking:
        ...
        if self._route_time_since_flush >= self._ROUTE_LOG_EVERY:
            n = self._route_time_since_flush
            logger.info(
                f"route-stats: calls={n} total={self._route_time_total_s:.3f}s "
