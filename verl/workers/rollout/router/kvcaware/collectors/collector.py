...
class Collector:
    ...
    def _write_metrics_update(self, update: MetricsUpdate) -> None:
        ...
        if update.is_delta:
            ...
            # Turn fires on a dispatch (DISPATCHED_COUNT bumped), not on
            # request_id presence — a request_id-carrying non-dispatch update
            # must not be mis-counted as a dispatch.
            if MetricKey.DISPATCHED_COUNT in update.metrics:
                if update.request_id is None:
                    logger.debug("dispatch (DISPATCHED_COUNT) update missing request_id — skipping turn")
                else:
                    turn = self._data_store.incr_per_request(update.request_id, "turn")
                    self._data_store.incr_metric(update.node_id, MetricKey.TURN_SUM, turn)
