...
def _router_config():
    """Build a rollout-shaped config carrying the kvcaware ``strategy`` node.

    Mirrors the production path: ``get_router_handle`` reads
    ``router_strategy`` (→ "kvcaware") and ``max_num_seqs`` from the top
    level, then passes the ``strategy`` node to ``KVCAwareConfig.from_config``.
    """
    return OmegaConf.create({
        "router_strategy": "kvcaware",
        "max_num_seqs": 16,
        "strategy": _KVCAWARE_STRATEGY,
    })
