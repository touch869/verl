    ...
    def test_k14_yaml_split_loading(self):
        ...
        with initialize_config_dir(config_dir=config_dir, version_base=None):
            cfg = compose(
                ...
                overrides=["rollout/router@actor_rollout_ref.rollout.strategy=kvcaware", "actor_rollout_ref.rollout.router_strategy=kvcaware"],
            )

        strat = cfg.actor_rollout_ref.rollout.strategy
...
class TestDropInIntegration:
    ...
    def _make_full_config(self) -> OmegaConf:
        ...
        the rollout ``strategy`` node (selected via ``router@strategy: kvcaware``).
    ...
    def test_d05_compose_expands_defaults(self):
        ...
        Description: compose ppo_trainer with +rollout@...strategy=kvcaware
        Expectation: strategies/collector/cache_store all present under rollout.strategy
        """
        from pathlib import Path
    ...
    def test_d14_yaml_file_e2e(self):
        ...
        # nests under rollout.strategy; from_config takes that sub-tree directly.
        strat = cfg.actor_rollout_ref.rollout.strategy
